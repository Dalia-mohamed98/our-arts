...
function dokan_seller_sales_statement() {
    ...
    <form method="get" class="dokan-form-inline report-filter dokan-clearfix" action="">
        <div class="dokan-form-group">
            <label for="from"><?php _e( 'من:', 'dokan' ); ?></label> <input type="text" class="datepicker" name="start_date" id="from" readonly="readonly" value="<?php echo date_i18n( get_option( 'date_format' ), strtotime( $start_date ) ); ?>" />
        </div>

        <div class="dokan-form-group">
            <label for="to"><?php _e( 'إلى:', 'dokan' ); ?></label>
            ...
            <input type="submit" name="dokan_report_filter" class="dokan-btn dokan-btn-success dokan-btn-sm dokan-theme" value="<?php _e( 'عرض', 'dokan' ); ?>" />
        </div>
        <input type="submit" name="dokan_statement_export_all"  class="dokan-btn dokan-right dokan-btn-sm dokan-btn-danger dokan-btn-theme" value="<?php esc_attr_e( 'تصدير الكل', 'dokan' ); ?>">
    </form>
    <?php
    global $wpdb;
    ...
    <table class="table table-striped">
        <thead>تاريخ الرصيد
            <tr>
                <th><?php _e( 'تاريخ الرصيد', 'dokan' ); ?></th>
                <th><?php _e( 'Trn تاريخ', 'dokan' ); ?></th>
                <th><?php _e( 'ID', 'dokan' ); ?></th>
                <th><?php _e( 'نوع', 'dokan' ); ?></th>
                <th><?php _e( 'مدين', 'dokan' ); ?></th>
                <th><?php _e( 'ائتمان', 'dokan' ); ?></th>
                <th><?php _e( 'رصيد حساب', 'dokan' ); ?></th>
            </tr>
        </thead>
        <tbody>
            <?php if( $opening_balance ) { ?>
                <tr>
                    ...
                    <td><?php echo '--'; ?></td>
                    <td><?php echo '--'; ?></td>
                    <td><?php _e( 'الرصيد الافتتاحي' , 'dokan' ); ?></td>
                    <td><?php echo '--'; ?></td>
                    <td><?php echo '--'; ?></td>
            ...
            <?php } if( count( $statements ) ) {
                ...
                <tr>
                    ...
                    <td></td>
                    <td></td>
                    <td><b><?php _e( 'الإجمالي :', 'dokan' ); ?></b></td>
            ...
            <?php
                ...
                ?>
                <tr>
                    <td colspan="6"><?php _e( 'لم يتم العثور على نتيجة!', 'dokan' ); ?></td>
                </tr>
                <?php
            }
...
function dokan_report_sales_overview( $start_date, $end_date, $heading = '' ) {
    ...
    $legend = apply_filters( 'dokan-seller-dashboard-reports-left-sidebar', array(
        'sales_in_this_period' => array(
            'title' => sprintf( __( '%s المبيعات في هذه الفترة', 'dokan' ), '<strong>' . wc_price( $total_sales ) . '</strong>' ),
        ),

        'average_daily_sales' => array(
            'title' => sprintf( __( '%s متوسط المبيعات اليومية', 'dokan' ), '<strong>' . wc_price( $average_sales ) . '</strong>' ),
        ),

        'orders_placed' => array(
            'title' => sprintf( __( '%s  صنفت الطلبات ', 'dokan' ), '<strong>' . $total_orders . '</strong>' ),
        ),

        'items_purchased' => array(
            'title' => sprintf( __( '%s منتجات تم شراؤها', 'dokan' ), '<strong>' . $total_items . '</strong>' ),
        ),

//         'charged_for_shipping' => array(
//             'title' => sprintf( __( '%s مكلف للشحن', 'dokan' ), '<strong>' . wc_price( $total_shipping ) . '</strong>' ),
//         ),

//         'worth_of_coupons_used' => array(
//             'title' => sprintf( __( '%s قيمة الكوبونات المستخدمة', 'dokan' ), '<strong>' . wc_price( $total_coupons ) . '</strong>' ),
//         ),
    ) );
    ?>
...
function dokan_sales_overview_chart_data( $start_date, $end_date, $group_by ) {
    ...
    <script type="text/javascript">
        jQuery(function($) {
            ...
            var series = [
                {
                    label: "<?php echo esc_js( __( 'المبيعات', 'dokan' ) ) ?>",
                ...
                {
                    label: "<?php echo esc_js( __( 'عدد الطلبات', 'dokan' ) ) ?>",
                    ...
                    shadowSize: 0,
                    append_tooltip: " <?php echo __( 'مبيعات', 'dokan' ); ?>"
                },
            ];
...
function dokan_top_sellers() {
    ...
    <form method="post" action="" class="report-filter dokan-form-inline dokan-clearfix">
        ...
        <input type="submit" class="dokan-btn dokan-btn-success dokan-btn-sm dokan-theme" value="<?php _e( 'عرض', 'dokan' ); ?>" />
    </form>
    ...
    <table class="table table-striped">
        <thead>
            <tr>
                <th><?php _e( 'منتج', 'dokan' ); ?></th>
                <th><?php _e( 'مبيعات', 'dokan' ); ?></th>
            </tr>
        </thead>
        <tbody>
            <?php
...
function dokan_top_earners() {
    ...
    <form method="post" action="" class="report-filter dokan-form-inline dokan-clearfix">
        <div class="dokan-form-group">
            <label for="from"><?php _e( 'من:', 'dokan' ); ?></label>
            <input type="text" class="datepicker" name="start_date" id="from" readonly="readonly" value="<?php echo date_i18n( get_option( 'date_format' ), strtotime( $start_date ) ); ?>" />
        </div>
    ...
    <table class="table table-striped">
        <thead>
            <tr>
                <th><?php _e( 'المنتج', 'dokan' ); ?></th>
                <th colspan="2"><?php _e( 'المبيعات', 'dokan' ); ?></th>
                <th><?php _e( 'المكسب', 'dokan' ); ?></th>
            </tr>
        </thead>
        <tbody>
            <?php
                ...
                foreach ( $found_products as $product_id => $sales ) {
                    ...
                    if ( $product_title ) {
                        ...
                        $product_name = __( 'المنتج لم يعد موجودا', 'dokan' );
                        $orders_link = admin_url( 'edit.php?s&post_status=all&post_type=shop_order&action=-1&s=&shop_order_status=' . implode( ",", apply_filters( 'woocommerce_reports_order_statuses', array( 'wc-completed', 'wc-processing', 'wc-on-hold' ) ) ) );
                    }
