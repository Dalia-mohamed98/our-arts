...
<tr class="<?php echo esc_attr( $tr_class ); ?>">
    ...
    <td data-title="<?php esc_attr_e( 'الصورة', 'dokan-lite' ); ?>">
        <?php if ( current_user_can( 'dokan_edit_product' ) ): ?>
            <a href="https://our-arts.com/dashboard/products/?product_id=<?php echo absint( dokan_get_prop( $product, 'id' )). '&action=edit'; //echo esc_url( dokan_edit_product_url( $post->ID ) ); ?>"><?php echo $product->get_image(); ?></a>
        <?php else: ?>
    ...
    <td class="post-status" data-title="<?php esc_attr_e( 'الحالة', 'dokan-lite' ); ?>">
    ...
    <td data-title="<?php esc_attr_e( 'المخزون', 'dokan-lite' ); ?>">
        <?php
    ...
    <td data-title="<?php esc_attr_e( 'السعر', 'dokan-lite' ); ?>">
        <?php
    ...
    <td data-title="<?php esc_attr_e( 'المكسب', 'dokan-lite' ); ?>">
        <?php
    ...
    <td data-title="<?php esc_attr_e( 'النوع', 'dokan-lite' ); ?>">
        <?php
        if( dokan_get_prop( $product, 'product_type' , 'get_type') == 'grouped' ):
            echo '<span class="product-type tips grouped" title="' . esc_html__( 'مجمعة', 'dokan-lite' ) . '"></span>';
        elseif ( dokan_get_prop( $product, 'product_type' , 'get_type') == 'external' ):
            echo '<span class="product-type tips external" title="' . esc_html__( 'الخارجية / التابعة ل', 'dokan-lite' ) . '"></span>';
        elseif ( dokan_get_prop( $product, 'product_type' , 'get_type') == 'simple' ):

            if ( $product->is_virtual() ) {
                echo '<span class="product-type tips virtual" title="' . esc_html__( 'افتراضية', 'dokan-lite' ) . '"></span>';
            } elseif ( $product->is_downloadable() ) {
                echo '<span class="product-type tips downloadable" title="' . esc_html__( 'للتحميل', 'dokan-lite' ) . '"></span>';
            } else {
                echo '<span class="product-type tips simple" title="' . esc_html__( 'بسيط', 'dokan-lite' ) . '"></span>';
            }

            elseif ( dokan_get_prop( $product, 'product_type' , 'get_type') == 'variable' ):
                echo '<span class="product-type tips variable" title="' . esc_html__( 'متغير', 'dokan-lite' ) . '"></span>';
            else:
    ...
    <td data-title="<?php esc_attr_e( 'المشاهدات', 'dokan-lite' ); ?>">
        <?php echo (int) get_post_meta( $post->ID, 'pageview', true ); ?>
    </td>
    <td class="post-date" data-title="<?php esc_attr_e( 'التاريخ', 'dokan-lite' ); ?>">
        <?php
        if ( '0000-00-00 00:00:00' == $post->post_date ) {
            ...
            if ( $time_diff > 0 && $time_diff < 24 * 60 * 60 ) {
                $h_time = sprintf( __( '%s في الماضي', 'dokan-lite' ), human_time_diff( $time ) );
            } else {
        ...
        if ( 'publish' == $post->post_status ) {
            esc_html_e( 'منشورة', 'dokan-lite' );
        } elseif ( 'future' == $post->post_status ) {
            if ( $time_diff > 0 ) {
                echo '<strong class="attention">' . esc_html__( 'الجدول الزمني الضائع', 'dokan-lite' ) . '</strong>';
            } else {
                esc_html_e( 'متجدول', 'dokan-lite' );
            }
        } else {
            esc_html_e( 'آخر تعديل', 'dokan-lite' );
        }
        ?>
        </div>
    </td>
