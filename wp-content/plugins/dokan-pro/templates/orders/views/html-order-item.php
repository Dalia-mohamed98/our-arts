...
<tr class="item <?php echo apply_filters( 'woocommerce_admin_html_order_item_class', ( ! empty( $class ) ? $class : '' ), $item ); ?>" data-order_item_id="<?php echo $item_id; ?>">
	...
	<td class="thumb" width="15%" >
		<?php if ( $_product ) : ?>
			<a target="_blank" href="<?php echo get_permalink(absint( dokan_get_prop( $_product, 'id' ))); //echo esc_url( admin_url( 'post.php?post=' . absint( dokan_get_prop( $_product, 'id' ) ) . '&action=edit' ) ); ?>" class="tips" data-tip="<?php
	...
	<td class="name" data-sort-value="<?php echo esc_attr( $item['name'] ); ?>">
		...
		<?php if ( $_product ) : ?>
			<a target="_blank" href="<?php echo get_permalink(absint( dokan_get_prop( $_product, 'id' ))); //echo esc_url( dokan_edit_product_url( absint( dokan_get_prop( $_product, 'id' ) ) ) ); ?>">
