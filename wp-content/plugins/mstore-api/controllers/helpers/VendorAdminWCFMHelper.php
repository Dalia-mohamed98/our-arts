...
class VendorAdminWCFMHelper
{
    ...
    public function get_vendor_profile($user_id)
    {
        ...
        $vendor_data['logo'] = wp_get_attachment_image_src($vendor_data['gravatar']) [0];
        $vendor_data['banner'] = wp_get_attachment_image_src($vendor_data['banner']) [0];
        $vendor_data['mobile_banner'] = wp_get_attachment_image_src($vendor_data['mobile_banner']) [0];
        $vendor_data['list_banner'] = wp_get_attachment_image_src($vendor_data['list_banner']) [0];
        $data = array();
    ...
    public function update_vendor_profile($request, $user_id)
    {
        ...
        if (isset($data['logo']))
        {
            $img_id = $this->upload_image_from_mobile($data['logo'], $count, $user_id);
            $count = $count + 1;
        ...
        {
            $img_id = $this->upload_image_from_mobile($data['mobile_banner'], $count, $user_id);
            $count++;
        ...
        {
            $img_id = $this->upload_image_from_mobile($data['banner'], $count, $user_id);
            $count++;
        ...
        {
            $vendor_data['banner_slider'] = array();
            foreach ($data['banner_slider'] as $item)
            {
                if ($item['type'] == 'asset')
                {
                    $img_id = $this->upload_image_from_mobile($item['image'], $count, $user_id);
                }
                if ($item['type'] == 'url')
                {
        ...
        {
            $img_id = $this->upload_image_from_mobile($data['list_banner'], $count, $user_id);
            $count++;
            $vendor_data['list_banner'] = $img_id;
            $vendor_data['list_banner_type'] = $data['list_banner_type'];
        }

        if (isset($data['list_banner_video']) && isset($data['list_banner_type']))
        {
        ...
        $vendor_data['store_hide_description'] = $data['store_hide_description'];
    ...
    public function flutter_update_order_status($request, $user_id)
    {
        ...
        if (wcfm_is_vendor($user_id)) add_filter('woocommerce_new_order_note_data', array(
            $WCFM->wcfm_marketplace,
            'wcfm_update_comment_vendor'
        ) , 10, 2);
        ...
        if (wcfm_is_vendor($user_id)) remove_filter('woocommerce_new_order_note_data', array(
            $WCFM->wcfm_marketplace,
            'wcfm_update_comment_vendor'
        ) , 10, 2);
        ...
        $WCFM
            ->wcfm_notification
            ->wcfm_send_direct_message(-2, 0, 1, 0, $wcfm_messages, 'status-update');
        ...
        if ($reviewid)
        {
            ...
            if ($review_data && !empty($review_data) && is_object($review_data))
            {
                ...
                { // On Approve
                    ...
                    $wpdb->update("{$wpdb->prefix}wcfm_marketplace_reviews", array(
                        'approved' => 1
                    ) , array(
                        'ID' => $reviewid
                    ) , array(
    ...
    function wcfm_query_time_range_filter($sql, $time, $interval = '7day', $start_date = '', $end_date = '', $table_handler = 'commission')
    {
                    ...
                    foreach ($gross_sales_whole_week as $net_sale_whole_week)
                    {
                        if ($net_sale_whole_week->order_item_id)
                        {
                            try
                            {
                                ...
                                if ($WCMp
                                    ->vendor_caps
                                    ->vendor_payment_settings('give_tax'))
                                {
                                    $gross_sales += (float)sanitize_text_field($line_item->get_total_tax());
                                ...
                                if ($WCMp
                                    ->vendor_caps
                                    ->vendor_payment_settings('give_shipping'))
                                {
                ...
                if (!empty($gross_sales_whole_week))
                {
                    ...
                    if (!empty($gross_commission_ids))
                    {
                        try
                        {
                            ...
                            {
                                $gross_sales = (float)$WCFMmp
                                    ->wcfmmp_commission
                                    ->wcfmmp_get_commission_meta_sum($gross_commission_ids, 'gross_total');
                            }
                            else
                            {
                                $gross_sales = (float)$WCFMmp
                                    ->wcfmmp_commission
                                    ->wcfmmp_get_commission_meta_sum($gross_commission_ids, 'gross_sales_total');
                            }
        ...
        function wcfm_get_commission_by_vendor($vendor_id = '', $interval = '7day', $is_paid = false, $order_id = 0, $filter_date_form = '', $filter_date_to = '')
        {
            ...
            elseif ($marketplece == 'wcmarketplace')
            {
                ...
                $vendor_handler = 'vendor_id';
                $table_handler = 'commission';
                if ($is_paid) $time = 'commission_paid_date';
                else $time = 'created';
            }
            elseif ($marketplece == 'wcpvendors')
            {
                ...
                $vendor_handler = 'vendor_id';
                $table_handler = 'commission';
                if ($is_paid) $time = 'paid_date';
                else $time = 'order_date';
            }
            elseif ($marketplece == 'dokan')
            {
                $order_status = apply_filters('wcfm_dokan_allowed_order_status', array(
                    'completed',
                    'processing',
                    'on-hold'
                ));
            ...
            if (!$order_id) $sql = $this->wcfm_query_time_range_filter($sql, $time, $interval, $filter_date_form, $filter_date_to, $table_handler);
