...
class Dokan_Template_Dashboard {
    ...
    function __construct() {
        ...
        add_action( 'dokan_dashboard_left_widgets', array( $this, 'get_orders_widgets' ), 5 );
        add_action( 'dokan_dashboard_left_widgets', array( $this, 'get_products_widgets' ), 5 );
        add_action( 'dokan_dashboard_left_widgets', array( $this, 'get_sales_report_chart_widget' ), 10 );
    ...
    public function get_orders_widgets() {
        ...
        $order_data = array(
            array(
				'value' => $this->orders_count->{'wc-delivered'},
				'color' => '#73a724',
				'label' => __( 'المستلمة', 'dokan-lite' ),
			),
            array(
				'value' => $this->orders_count->{'wc-on-hold'},
				'color' => '#999',
				'label' => __( 'بانتظار الموافقة', 'dokan-lite' ),
			),
            array(
				'value' => $this->orders_count->{'wc-pickedup'},
				'color' => '#21759b',
				'label' => __( 'المشحونة', 'dokan-lite' ),
			),
            array(
				'value' => $this->orders_count->{'wc-cancelled'},
				'color' => '#d54e21',
				'label' => __( 'الملغية', 'dokan-lite' ),
			),
			 array(
				'value' => $this->orders_count->{'wc-processing'},
				'color' => '#5bc0de',
				'label' => __( 'بانتظار البوليصة', 'dokan-lite' ),
			),
            array(
				'value' => $this->orders_count->{'wc-completed'},
				'color' => 'orange',
				'label' => __( 'جاهز للشحن', 'dokan-lite' ),
			),
			  array(
				'value' => $this->orders_count->{'wc-refunded'},
				'color' => '#e6db55',
				'label' => __( 'مرتجع', 'dokan-lite' ),
			),
