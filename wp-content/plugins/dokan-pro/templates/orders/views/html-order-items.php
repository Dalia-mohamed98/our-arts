    ...
    <table cellpadding="0" cellspacing="0" class="woocommerce_order_items dokan-table dokan-table-strip">
        <thead>
            <tr>
                ...
                <th class="item sortable" colspan="2" data-sort="string-ins"><?php _e( 'المنتج', 'dokan' ); ?></th>

                <?php do_action( 'woocommerce_admin_order_item_headers' ); ?>

                <th class="item_cost sortable" data-sort="float"><?php _e( 'السعر', 'dokan' ); ?></th>
                <th class="quantity sortable" data-sort="int"><?php _e( 'الكمية', 'dokan' ); ?></th>
                <th class="line_cost sortable" data-sort="float"><?php _e( 'الإجمالي', 'dokan' ); ?></th>
...
<div class="wc-order-data-row wc-order-totals-items wc-order-items-editable">
        ...
        if ( $coupons ) {
            ...
            <div class="wc-used-coupons">
                <ul class="wc_coupon_list"><?php
                    echo '<li><strong>' . __( 'كوبون استخدم', 'dokan' ) . '</strong></li>';
    ...
    <table class="wc-order-totals">
        ...
        <tr>
            <td><?php _e( 'مجموع سعر المنتجات', 'dokan' ); ?>:</td>
            <td class="total">
                <?php 
                 $order_shipping = $order->get_total_shipping();
                 $order_total = $data['_order_total'][0] - $order_shipping;
                 ?>
                <div class="view"><?php echo  wc_price($order_total) //$order->get_formatted_order_total(); ?></div>
        ...
        <tr>
            <td><?php _e( 'الخصم', 'dokan' ); ?> <span class="tips" data-tip="<?php _e( 'This is the total discount. Discounts are defined per line item.', 'dokan' ); ?>">[?]</span>:</td>
        ...
        <tr>
            <td><?php _e( 'رسوم أور آرتس', 'dokan' ); ?> <span class="tips" data-tip="<?php _e( 'هذه رسوم أور آرتس المستحقة.', 'dokan' ); ?>">[?]</span>:</td>
            <td class="total refunded-total">
                -
                <?php 
                    $seller_id  = dokan_get_seller_id_by_order( $order->get_id() );
                    $admin_commission   = dokan_get_admin_commission_by( $order, $seller_id ) - $order_shipping;
                    echo wc_price( $admin_commission, array( 'currency' => dokan_replace_func( 'get_order_currency', 'get_currency', $order ) ) ); ?>
            </td>
            <td width="1%"></td>
        </tr>

        <tr>
            <td><?php _e( 'المجموع الاجمالي', 'dokan' ); ?>:</td>
            <td class="total">
             <?php
                $net_amount = $order->get_total() - $admin_commission - $order_shipping;
                echo wc_price( $net_amount, array( 'currency' => dokan_replace_func( 'get_order_currency', 'get_currency', $order ) ) ); 
             ?>
            </td>
            <td><?php if ( $order->is_editable() ) : ?><div class="wc-order-edit-line-item-actions"><a class="edit-order-item" href="#"></a></div><?php endif; ?></td>
        </tr>
