...
function dokan_content_nav( $nav_id, $query = null ) {
    ...
    <nav role="navigation" id="<?php echo esc_attr( $nav_id ); ?>" class="<?php echo esc_attr( $nav_class ); ?>">
        ...
        <?php if ( is_single() ) : // navigation links for single posts  ?>
            ...
            <li class="previous">
                <?php previous_post_link( '%link', _x( '&larr;', 'رابط الصفحة السابق', 'dokan-lite' ) . ' %title' ); ?>
            </li>
            <li class="next">
                <?php next_post_link( '%link', '%title ' . _x( '&rarr;', 'رابط الصفحة التالي', 'dokan-lite' ) ); ?>
            </li>
...
function dokan_page_navi( $before = '', $after = '', $wp_query ) {
    ...
    if ( $prevposts ) {
        ...
        echo '<li class="disabled"><a href="#">' . esc_html__( '&larr; السابق', 'dokan-lite' ) . '</a></li>';
    }
...
function dokan_product_dashboard_errors() {
    ...
    switch ( $type ) {
        case 'product_deleted':
            dokan_get_template_part( 'global/dokan-success', '', array( 'deleted' => true, 'message' => __( 'تم حذف المنتج بنجاح', 'dokan-lite' ) ) );
            break;

        case 'error':
            dokan_get_template_part( 'global/dokan-error', '', array( 'deleted' => false, 'message' =>  __( 'هناك خطأ ما!', 'dokan-lite' ) ) );
            break;
    ...
    <ul class="list-inline order-statuses-filter">
        ...
        <li<?php echo $status_class == 'wc-completed' ? ' class="active"' : ''; ?>>
            ...
            <a href="<?php echo esc_url( add_query_arg( $complete_order_url, $orders_url ) ); ?>">
                <?php printf( esc_html__( 'مكتمل (%d)', 'dokan-lite' ), esc_attr( $orders_counts->{'wc-completed'} ) ); ?></span>
            </a>
        </li>
        <li<?php echo $status_class == 'wc-processing' ? ' class="active"' : ''; ?>>
            ...
            <a href="<?php echo esc_url( add_query_arg( $processing_order_url, $orders_url ) ); ?>">
                <?php printf( esc_html__( 'معالجة (%d)', 'dokan-lite' ), esc_attr( $orders_counts->{'wc-processing'} ) ); ?></span>
            </a>
        </li>
        <li<?php echo $status_class == 'wc-on-hold' ? ' class="active"' : ''; ?>>
            ...
            <a href="<?php echo esc_url( add_query_arg( $on_hold_order_url, $orders_url ) ); ?>">
                <?php printf( esc_html__( 'معلقة (%d)', 'dokan-lite' ), esc_attr( $orders_counts->{'wc-on-hold'} ) ); ?></span>
            </a>
        </li>
        <li<?php echo $status_class == 'wc-pending' ? ' class="active"' : ''; ?>>
            ...
            <a href="<?php echo esc_url( add_query_arg( $pending_order_url, $orders_url ) ); ?>">
                <?php printf( esc_html__( 'قيد الإنتظار (%d)', 'dokan-lite' ), esc_attr( $orders_counts->{'wc-pending'} ) ); ?></span>
            </a>
        </li>
        <li<?php echo $status_class == 'wc-canceled' ? ' class="active"' : ''; ?>>
            ...
            <a href="<?php echo esc_url( add_query_arg( $canceled_order_url, $orders_url ) ); ?>">
                <?php printf( esc_html__( 'ملغي (%d)', 'dokan-lite' ), esc_attr( $orders_counts->{'wc-cancelled'} ) ); ?></span>
            </a>
        </li>
        <li<?php echo $status_class == 'wc-refunded' ? ' class="active"' : ''; ?>>
            ...
            <a href="<?php echo esc_url( add_query_arg( $refund_order_url, $orders_url ) ); ?>">
                <?php printf( esc_html__( 'مرتجع (%d)', 'dokan-lite' ), esc_attr( $orders_counts->{'wc-refunded'} ) ); ?></span>
            </a>
        </li>
        <li<?php echo $status_class == 'wc-failed' ? ' class="active"' : ''; ?>>
            ...
            <a href="<?php echo esc_url( add_query_arg( $failed_order_url, $orders_url ) ); ?>">
                <?php printf( esc_html__( 'فشل (%d)', 'dokan-lite' ), esc_attr( $orders_counts->{'wc-failed'} ) ); ?></span>
            </a>
        </li>
...
function dokan_get_dashboard_nav() {
    ...
    $urls = array(
        ...
        'withdraw' => array(
            'title'      => __( 'سحب النقود', 'dokan-lite'),
    ...
    $settings = array(
        'title' => sprintf( '%s <i class="fa fa-angle-right pull-right"></i>', __( 'الإعدادات', 'dokan-lite') ),
    ...
    $settings_sub = array(
        'back' => array(
            'title' => __( 'العودة الي الرئيسية ', 'dokan-lite'),
            'icon'  => '<i class="fa fa-long-arrow-left"></i>',
            'url'   => dokan_get_navigation_url(),
            'pos'   => 10
        ),
        'store' => array(
            'title'      => __( 'المتجر', 'dokan-lite'),
...
function dokan_dashboard_nav( $active_menu = '' ) {
    ...
    $common_links = '<li class="dokan-common-links dokan-clearfix">
            <a title="' . __( 'زيارة المتجر', 'dokan-lite' ) . '" class="tips" data-placement="top" href="' . dokan_get_store_url( dokan_get_current_user_id() ) .'" target="_blank"><i class="fa fa-shopping-cart"></i></a>
            <a title="' . __( 'تعديل الحساب', 'dokan-lite' ) . '" class="tips" data-placement="top" href="' . dokan_get_navigation_url( 'edit-account' ) . '"><i class="fa fa-user"></i></a>
            <a title="' . __( 'تسجيل الخروج', 'dokan-lite' ) . '" class="tips" data-placement="top" href="' . wp_logout_url( home_url() ) . '"><i class="fa fa-power-off"></i></a>
        </li>';
