...
<div class="dokan-order-filter-serach">
    <form action="" method="GET" class="dokan-left" style="margin: 0;">
        <div class="dokan-form-group" style="display: inline-flex;">
            <input type="text" class="datepicker" style="width:120px; padding-bottom:7px" name="order_date" id="order_date_filter" placeholder="<?php esc_attr_e( 'تنقية بالتاريخ', 'dokan-lite' ); ?>" value="<?php echo esc_attr( $filter_date ); ?>">
            <select name="customer_id" id="dokan-filter-customer" style="width:200px" class="dokan-form-control"  data-allow_clear="true" data-placeholder="<?php esc_attr_e( 'تنقية بالعميل', 'dokan-lite' ); ?>">
                <option value="<?php echo esc_attr( $user_id ); ?>" selected="selected"><?php echo wp_kses_post( $user_string ); ?><option>
            </select>
            <input type="submit" name="dokan_order_filter" class="dokan-btn dokan-btn-sm dokan-btn-danger dokan-btn-theme" value="<?php esc_attr_e( 'تنقية', 'dokan-lite' ); ?>">
            <input type="hidden" name="order_status" value="<?php echo  esc_attr( $order_status ); ?>">
        </div>
    </form>

    <form action="" method="POST" class="dokan-right" style="margin: 0;">
