...
function prefix_admin_add_setting() {
	...
	if ( isset( $_REQUEST['settingsubmit'] ) ) 
	{
		...
		{
			//$wpdb->show_errors();
			$updateQuery = $wpdb->update( 'wp_logixgridsetting', array( 'secureKey' => $secureKey, 'accessKey' => $accessKey, 'customerCode' => $customerCode, 'serviceCode' => $serviceCode, 'sourceCountry' => $sourceCountry, 'apiURL' => $apiURL, 'pluginName' => $pluginName), array( 'ID' => 1));
